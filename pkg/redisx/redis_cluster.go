...
var ErrRedisIsDown = errors.New("Redis is enter down or ws not configureds")
...
func connectSingleton(cache bool, config *Config) bool {
	if singleton(cache) == nil {
		logger.Debug("Connection to redis clusters")
		if cache {
...
func clusterConnectionIsOpen(cluster RedisCluster) bool {
	...
	if err := c.Set(testKey, "test", time.Second).Err(); err != nil {
		logger.Warnf("Error trying to set test key: %s", err.Error())

		return false
	}
	if _, err := c.Get(testKey).Result(); err != nil {
		logger.Warnf("Error trying to get test key: %s", err.Error())
		return false
	}
...
func (r *RedisCluster) GetKey(keyName string) (string, error) {
	...
	if err != nil {
		logger.Debugf("Error trying to get value: %s", err.Error())
...
func (r *RedisCluster) GetExp(keyName string) (int64, error) {
	logger.Debugf("Getting exp for key: %s", r.fixKey(keyName))
	if err := r.up(); err != nil {
		return 0, err
	}
	...
	if err != nil {
		logger.Errorf("Error trying to get TTL: ", err.Error())
...
func (r *RedisCluster) SetExp(keyName string, timeout time.Duration) error {
	...
	if err != nil {
		logger.Errorf("Could not EXPIRE key: %s", err.Error())
	}
...
func (r *RedisCluster) SetKey(keyName, session string, timeout time.Duration) error {
	logger.Debugf("[STORE] SET Raw key is: %s", keyName)
	logger.Debugf("[STORE] Setting key: %s", r.fixKey(keyName))
	...
	if err != nil {
		logger.Errorf("Error trying to set value: %s", err.Error())
...
func (r *RedisCluster) Decrement(keyName string) {
	...
	logger.Debugf("Decrementing key: %s", keyName)
	if err := r.up(); err != nil {
		return
	}
	err := r.singleton().Decr(keyName).Err()
	if err != nil {
		logger.Errorf("Error trying to decrement value: %s", err.Error())
	}
}
...
func (r *RedisCluster) IncrememntWithExpire(keyName string, expire int64) int64 {
	logger.Debugf("Incrementing raw key: %s", keyName)
	if err := r.up(); err != nil {
		return 0
	}
	...
	if err != nil {
		logger.Errorf("Error trying to increment value: %s", err.Error())
	} else {
		logger.Debugf("Incremented key: %s, val is: %d", fixedKey, val)
	}

	if val == 1 && expire > 0 {
		logger.Debug("--> Setting Expire")
...
func (r *RedisCluster) GetKeys(filter string) []string {
	...
	logger.Debugf("[STORE] Getting list by: %s", searchStr)
	...
	if err != nil {
		logger.Errorf("Error while fetching keys: %s", err)
	...
	case *redis.ClusterClient:
		{
			...
			_, err := pipe.Exec()
			if err != nil && !errors.Is(err, redis.Nil) {
				logger.Errorf("Error trying to get client keys: %s", err.Error())
...
func (r *RedisCluster) DeleteKey(keyName string) bool {
	...
	logger.Debugf("DEL Key was: %s", keyName)
	logger.Debugf("DEL Key became: %s", r.fixKey(keyName))
	n, err := r.singleton().Del(r.fixKey(keyName)).Result()
	if err != nil {
		logger.Errorf("Error trying to delete key: %s", err.Error())
	}
...
func (r *RedisCluster) DeleteAllKeys() bool {
	...
	if err != nil {
		logger.Errorf("Error trying to delete keys: %s", err.Error())
	}
...
func (r *RedisCluster) DeleteScanMatch(pattern string) bool {
	...
	client := r.singleton()
	logger.Debugf("Deleting: %s", pattern)
	...
	if err != nil {
		logger.Errorf("SCAN command field with err: %s", err.Error())
	...
	if len(keys) > 0 {
		for _, name := range keys {
			logger.Infof("Deleting: %s", name)
			err := client.Del(name).Err()
			if err != nil {
				logger.Errorf("Error trying to delete key: %s - %s", name, err.Error())
			}
		}
		logger.Infof("Deleted: %d records", len(keys))
	} else {
		logger.Debug("RedisCluster called DEL - Nothing to delete")
	}
...
func (r *RedisCluster) StartPubSubHandler(channel string, callback func(interface{})) error {
	...
	if _, err := pubsub.Receive(); err != nil {
		logger.Errorf("Error while receiving pubsub message: %s", err.Error())
...
func (r *RedisCluster) GetAndDeleteSet(keyName string) []interface{} {
	logger.Debugf("Getting raw key set: %s", keyName)
	if err := r.up(); err != nil {
		return nil
	}
	logger.Debugf("keyName is: %s", keyName)
	fixedKey := r.fixKey(keyName)
	logger.Debugf("Fixed keyname is: %s", fixedKey)
	...
	if err != nil {
		logger.Errorf("Multi command failed: %s", err.Error())

		return nil
	}

	vals := lrange.Val()
	logger.Debugf("Analytics returned: %d", len(vals))
	if len(vals) == 0 {
		return nil
	}

	logger.Debugf("Unpacked vals: %d", len(vals))
...
func (r *RedisCluster) AppendToSet(keyName, value string) {
	fixedKey := r.fixKey(keyName)
	logger.Debug("Pushing to raw key list", logger.String("keyName", keyName))
	logger.Debug("Appending to fixed key list", logger.String("fixedKey", fixedKey))
	if err := r.up(); err != nil {
		return
	}
	if err := r.singleton().RPush(fixedKey, value).Err(); err != nil {
		logger.Errorf("Error trying to append to set keys: %s", err.Error())
	}
}
...
func (r *RedisCluster) Exists(keyName string) (bool, error) {
	fixedKey := r.fixKey(keyName)
	logger.Debug("Checking if exists", logger.String("keyName", fixedKey))

	exists, err := r.singleton().Exists(fixedKey).Result()
	if err != nil {
		logger.Errorf("Error trying to check if key exists: %s", err.Error())
...
func (r *RedisCluster) RemoveFromList(keyName, value string) error {
	...
	logger.Debug(
		...
		logger.String("keyName", keyName),
		logger.String("fixedKey", fixedKey),
...
func (r *RedisCluster) GetSet(keyName string) (map[string]string, error) {
	logger.Debugf("Getting from key set: %s", keyName)
	logger.Debugf("Getting from fixed key set: %s", r.fixKey(keyName))
	if err := r.up(); err != nil {
		return nil, err
	}
	val, err := r.singleton().SMembers(r.fixKey(keyName)).Result()
	if err != nil {
		logger.Errorf("Error trying to get key set: %s", err.Error())
...
func (r *RedisCluster) AddToSet(keyName, value string) {
	logger.Debugf("Pushing to raw key set: %s", keyName)
	logger.Debugf("Pushing to fixed key set: %s", r.fixKey(keyName))
	if err := r.up(); err != nil {
		return
	}
	err := r.singleton().SAdd(r.fixKey(keyName), value).Err()
	if err != nil {
		logger.Errorf("Error trying to append keys: %s", err.Error())
	}
}

// RemoveFromSet remove a value from key set.
func (r *RedisCluster) RemoveFromSet(keyName, value string) {
	logger.Debugf("Removing from raw key set: %s", keyName)
	logger.Debugf("Removing from fixed key set: %s", r.fixKey(keyName))
	if err := r.up(); err != nil {
		logger.Debug(err.Error())
	...
	if err != nil {
		logger.Errorf("Error trying to remove keys: %s", err.Error())
	}
}
...
func (r *RedisCluster) IsMemberOfSet(keyName, value string) bool {
	...
	if err != nil {
		logger.Errorf("Error trying to check set member: %s", err.Error())

		return false
	}

	logger.Debugf("SISMEMBER %s %s %v %v", keyName, value, val, err)
...
func (r *RedisCluster) SetRollingWindow(
	...
	logger.Debugf("keyName is: %s", keyName)
	now := time.Now()
	logger.Debugf("Now is: %v", now)
	onePeriodAgo := now.Add(time.Duration(-1*per) * time.Second)
	logger.Debugf("Then is: %v", onePeriodAgo)
	...
	logger.Debugf("Returned: %d", intVal)
...
func (r *RedisCluster) AddToSortedSet(keyName, value string, score float64) {
	...
	logger.Debug("Pushing raw key to sorted set", logger.String("keyName", keyName), logger.String("fixedKey", fixedKey))
...
func (r *RedisCluster) GetSortedSetRange(keyName, scoreFrom, scoreTo string) ([]string, []float64, error) {
	...
	logger.Debug(
		...
		logger.String("fixedKey", fixedKey),
		logger.String("scoreFrom", scoreFrom),
		logger.String("scoreTo", scoreTo),
	)
